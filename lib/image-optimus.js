...
function updateReqUrl(ext, req, res, next, pathname, extpos, vary){
	...
	return function(req, res, next){
		...
		if (ext === 'jpeg' || ext === 'jpg' || ext === 'png'){
			if (req.headers.accept.indexOf('image/webp') !== -1){
				updateReqUrl('webp', req, res, next, pathname, extpos, 'Accept');
			} else {
				...
				if ((is.chrome && agent.satisfies('>=23.0.0'))
					...
					updateReqUrl('jxr', req, res, next, pathname, extpos, 'User-Agent');
				} else {
					next();
				}
			}
